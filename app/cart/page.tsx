...
const Cart = () => {
  ...
  const { cartItems, removeFromCart } = useContext(ProductContext) as CountContext;
  ...
  const handleCheckOut = () => {
    const queryParams = new URLSearchParams({ cart: JSON.stringify(cartItems) }).toString();
      ...
      <div className="lg:py-10 xs:py-5 py-3 lg:px-14 xs:px-7 px-3 grid grid-cols-[2fr,1fr] gap-4">
        <div className="part 1">
          <div className="cart-nav sm:mb-10 mb-5 bg-[#F9F1E7] grid grid-cols-6 justify-items-center items-center sm:pl-0 pl-5 py-3">
            ...
            <p className="lg:text-16 sm:text-14 text-12 font-medium">Quantity</p>
            <p className="lg:text-16 sm:text-14 text-12 font-medium">Subtotal</p>
          </div>
            ...
            {cartItems.length === 0 ? (
              ...
              cartItems.map((item, index) => (
                ...
                >
                  ...
                  <h1 className="text-gray lg:text-16 sm:text-14 text-12">{item.title}</h1>
                  <p className="lg:text-16 sm:text-14 text-12">{item.price}</p>
                  <p className="pl-5">{item.quantity}</p>
                  <p className="lg:text-16 sm:text-14 text-12">{item.price * item.quantity}</p>
        ...
        <div className="part2 bg-[#F9F1E7] pt-5 lg:px-16 px-10 lg:pb-14 pb-10 text-center h-[18rem]">
          <h1 className="lg:text-30 text-24 font-semibold lg:mb-10 mb-6">Cart Totals</h1>
          <div className="space-y-6">
            ...
            <div className="flex justify-between">
              ...
              <p className="lg:text-20 text-18 font-medium text-[#B88E2F]">Rs. {totalAmount}</p>
            </div>
          </div>
          <div className="mt-8">
            <Button
