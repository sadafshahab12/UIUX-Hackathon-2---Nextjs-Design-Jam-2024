  ...
  return (
    <>
      {singleProduct.map((singleFurniture: ProductType, index: number) => {
        ...
        const titleCase = singleFurniture.tags[0].charAt(0).toUpperCase() + singleFurniture.tags[0].slice(1).toLowerCase();
        ...
        return (
          <div key={index} className="md:mt-[5rem] xs:mt-[4rem] mt-[3.8rem]">
            ...
            <ul className="flex-no-center sm:gap-7 xs:gap-5 gap-3 bg-[#F9F1E7] py-5 sm:px-24 xs:px-12 xss:px-5 px-3">
              <li>
                <Link href="/" className="font-medium text-gray sm:text-16 text-14">Home</Link>
              </li>
              <li><FontAwesomeIcon icon={faChevronRight} width={10} height={10} /></li>
              <li>
                <Link href="/shop" className="font-light text-gray sm:text-16 text-14">Shop</Link>
              </li>
              <li><FontAwesomeIcon icon={faChevronRight} width={10} height={10} /></li>
              <li>|</li>
            ...
            <div className="product-view lg:py-10 py-8 lg:px-14 sm:px-8 xs:px-10 px-5 flex md:flex-row flex-col gap-5">
              ...
              <div className="part1 flex sm:flex-row flex-col-reverse gap-4 md:justify-self-auto justify-self-center">
                <div className="img-col1 lg:space-y-5 sm:space-y-3 space-y-0 sm:block flex-center sm:gap-4 gap-3">
                  {singleFurniture.imageUrls.slice(0, 4).map((url: string, index: number) => (
                    <div
                      key={index}
                      className={`md:w-[100px] sm:w-[80px] xs:w-[100px] xss:w-[70px] w-[60px] md:h-[100px] sm:h-[80px] xs:h-[100px] xss:h-[70px] h-[60px] p-1 sm:rounded-[10px] rounded-[8px] cursor-pointer border border-slate-800 ${index === selectedImgIndex ? "border-2 border-[#B88E2F]" : "bg-[#F9F1E7]"}`}
              ...
              <div className="part2 md:space-y-4 space-y-2">
                ...
                <div className="flex-no-center gap-10">
                  ...
                  <p className="original-price sm:text-14 text-12 text-gray-400 line-through">
                    {singleFurniture.dicountPercentage > 0 && !singleFurniture.isNew && `$${singleFurniture.price}`}
                  </p>
                </div>

                {/* Stock Availability */}
                <div>
                  <p className={`${singleFurniture.isStock ? "text-green-700" : "text-red-500"}`}>
                    {singleFurniture.isStock ? `Stock: ${singleFurniture.stock}` : "Out of Stock"}
                  </p>
                </div>
                ...
                <div className="flex-between sm:flex-row flex-col gap-4 w-full">
                  {singleFurniture.availableForRental && (
                    <p className="inline text-12 bg-gradient-to-br from-slate-800 to-black text-white py-1 px-3 rounded-xl sm:w-auto w-full text-center">
                      Available for Rental
                    </p>
                  )}
                  {singleFurniture.rentalPricePerDay && (
                    <p className="text-sm bg-yellow-500 py-1 px-4 rounded-2xl sm:w-auto w-full text-center">
                ...
                <div className="buttons grid xss:grid-cols-3 grid-cols-2 lg:gap-5 gap-3 pt-4 w-full">
                  ...
                  <Link href={"/cart"}>
                    <button onClick={() => addToCart(singleFurniture, count)} className="border border-slate-800 py-3 lg:px-4 px-2 rounded-md lg:text-16 md:text-14 text-12 w-full">
                      Add To Cart
                    </button>
                  </Link>
